...
revoke insert, update, delete on table public.workbench_state from anon, authenticated;
grant select on table public.workbench_state to anon, authenticated;

drop policy if exists "Public can read workbench" on public.workbench_state;
create policy "Public can read workbench"
on public.workbench_state for select
to anon, authenticated
using (true);

insert into public.workbench_state (id, data)
values ('main', '{}'::jsonb)
